...
class WSv2 extends EventEmitter {
  ...
  _onWSNotification (arrN) {
    ...
    if (arrN[1] === 'on-req' || arrN[1] === 'fon-req') {
      if (arrN[1] === 'fon-req') { console.log('[ws] fon-req NOTIFICATION:', arrN); }
      const [,, cid] = arrN[4]
      const k = arrN[1] === 'on-req' ? `order-new-${cid}` : `forder-new-${cid}`;
    ...
    } else if (arrN[1] === 'oc-req' || arrN[1] === 'foc-req') {
      if (arrN[1] === 'foc-req') { console.log('[ws] foc-req NOTIFICATION:', arrN); }
      const [id] = arrN[4]
      const k = arrN[1] === 'oc-req' ? `order-cancel-${id}` : `forder-cancel-${id}`;
  ...
  submitOffer (order) {
    ...
    const packet = Object.assign({ cid: Date.now() }, order)

    debug(`creating offer ${JSON.stringify(packet)}`)
    this._sendOrderPacket([0, 'fon', null, packet])

    return this._getEventPromise(`forder-new-${packet.cid}`)
  }
  ...
  cancelOffer (order) {
    ...
    debug(`cancelling offer ${id}`)
    this._sendOrderPacket([0, 'foc', null, { id }])

    return this._getEventPromise(`forder-cancel-${id}`)
  }  
